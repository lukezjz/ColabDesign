...
class _af_loss:
  ...
  def _loss_affibody(self, inputs, outputs, aux):
    ...
    # supervised losses
    dgram_cce = (get_dgram_loss(I, O, copies=copies, aatype=I["aatype"], pos=np.arange(0, tL)) +
                 get_dgram_loss(I, O, copies=copies, aatype=I["aatype"], pos=np.arange(tL, tL + bL))) / 2
    aux["losses"].update({
      "dgram_cce": dgram_cce,
      "fape": get_fape_loss(I, O, copies=copies, clamp=opt["fape_cutoff"]),
      "rmsd": aln["rmsd"],
  ...
  def _loss_BRIL(self, inputs, outputs, aux):
    """get losses"""
    opt = inputs["opt"]
    pos = opt["pos"]
    ...
    I = {"aatype": aatype, "batch": inputs["batch"], "seq_mask": sub(inputs["seq_mask"])}
    O = {"distogram": dgram, "structure_module": {"final_atom_positions": atoms}}
    aln = get_rmsd_loss(I, O, include_L=False, copies=copies)

    # supervised losses
    ends = np.cumsum(self._pos_info["length"][self.order])
    frag1 = np.arange(0, ends[0])
    frag2 = np.arange(ends[0], ends[1])
    frag3 = np.arange(ends[1], ends[2])
    frag13 = np.concatenate([frag1, frag3])
    dgram_cce = (get_dgram_loss(I, O, copies=copies, aatype=I["aatype"], pos=frag13) +
                 get_dgram_loss(I, O, copies=copies, aatype=I["aatype"], pos=frag2)) / 2
    aux["losses"].update({
      "dgram_cce": dgram_cce,
      "fape": get_fape_loss(I, O, copies=copies, clamp=opt["fape_cutoff"]),
      "rmsd": aln["rmsd"],
...
# loss functions
####################
def get_dgram_loss(inputs, outputs, copies=1, aatype=None, return_mtx=False, pos=None):
  ...
  if pos is None:
    ...
    pred = outputs["distogram"]["logits"][pos,:][:,pos]
    weights = jnp.where(inputs["seq_mask"][pos], weights[pos], 0)
    x = x[pos]
